...
export function useAthletes(modalityId: number | null, eventId: string | null) {
  const { data: athletes, isLoading: isLoadingAthletes } = useQuery({
    ...
    queryFn: async () => {
      ...
      console.log('Parameters received:', { modalityId, eventId });
      ...
      try {
        ...
        console.log('All enrollments query result:', { 
        ...
        if (!allEnrollments || allEnrollments.length === 0) {
          console.log('No enrollments found for this modality/event combination');
          return [];
        }

        // Step 3: Filter confirmed enrollments
        const confirmedEnrollments = allEnrollments.filter(e => e.status === 'confirmado');
        console.log('Confirmed enrollments after filter:', {
          total: allEnrollments.length,
          confirmed: confirmedEnrollments.length,
          statuses: allEnrollments.map(e => ({ id: e.id, status: e.status }))
        });

        if (confirmedEnrollments.length === 0) {
          console.log('No confirmed enrollments found - all statuses:', allEnrollments.map(e => e.status));
          return [];
        }
        ...
        console.log('Users data query result:', {
          ...
          athleteIds,
          usersFound: usersData?.map(u => ({ id: u.id, nome: u.nome_completo })),
          error: usersError
        });
        ...
        if (!usersData || usersData.length === 0) {
          console.log('No users found for athlete IDs:', athleteIds);
          return [];
        }
