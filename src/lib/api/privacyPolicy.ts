...
export const fetchActivePrivacyPolicy = async (language: string = 'pt'): Promise<string> => {
  console.log(`🔍 API - Fetching active privacy policy for language: ${language}...`);

  try {
    // Tentativa com o cliente padrão do Supabase para acessar dados públicos
    const { data, error } = await supabase
      .from('termos_privacidade')
      .select('termo_texto_portugues, termo_texto_english, termo_texto_spanish')
      .eq('ativo', true)
    ...
    if (error) {
      ...
      if (error.code === 'PGRST301' || error.code === 'PGRST116') {
        ...
        const publicResponse = await fetch(`${supabaseUrl}/rest/v1/termos_privacidade?select=termo_texto_portugues,termo_texto_english,termo_texto_spanish&ativo=eq.true&order=data_criacao.desc&limit=1`, {
          headers: {
        ...
        if (publicResponse.ok) {
          ...
          if (publicData && publicData.length > 0) {
            ...
            const policy = publicData[0];
            return getPolicyByLanguage(policy, language);
          }
        } else {
    ...
    return getPolicyByLanguage(data, language);
  } catch (error: any) {
...
const getPolicyByLanguage = (data: any, language: string): string => {
  // Normalizar código de idioma (ex: pt-BR -> pt, en-US -> en)
  const normalizedLang = language.split('-')[0].toLowerCase();

  if (normalizedLang === 'en' && data.termo_texto_english) {
    return data.termo_texto_english;
  }
  if (normalizedLang === 'es' && data.termo_texto_spanish) {
    return data.termo_texto_spanish;
  }
  // Fallback para português
  return data.termo_texto_portugues || 'Conteúdo da política de privacidade não disponível.';
};
